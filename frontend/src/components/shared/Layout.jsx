    ...
    <NavLink to={to} onClick={onClick}
      className={({ isActive }) =>
        `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 group
         ${isActive ? 'bg-blue-500/20 border border-blue-500/30' : 'hover:bg-slate-800'}`
      }
  ...
  const SidebarContent = ({ mobile }) => (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', padding:'16px 12px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 4px 20px', borderBottom:'1px solid var(--border)', marginBottom:12 }}>
        <div style={{ width:36, height:36, background:'var(--bg-primary)', border:'2px solid #3b82f6', borderRadius:10, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
      ...
      <div style={{ borderTop:'1px solid var(--border)', paddingTop:12, marginTop:12, display:'flex', flexDirection:'column', gap:8 }}>
        <button onClick={toggleTheme} style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 12px', borderRadius:8, border:'1px solid var(--border)', background:'var(--bg-hover)', cursor:'pointer', color:'var(--text-secondary)', fontSize:13, fontWeight:500, transition:'all 0.15s', width:'100%' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 4px' }}>
          ...
          <button onClick={handleLogout} title="Sair" style={{ background:'none', border:'none', color:'var(--text-muted)', cursor:'pointer', padding:4, borderRadius:6 }}
